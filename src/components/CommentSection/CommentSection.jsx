...
import { API } from '../../config'; // Optional central API config
...
const CommentSection = ({ courseId, courseAuthorId }) => {
  ...
  const fetchComments = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API}/api/courses/${courseId}/comments`);
  ...
  const handleDelete = async (commentId) => {
    ...
    try {
      const response = await fetch(`${API}/api/courses/${courseId}/comments/${commentId}`, {
        method: 'DELETE',
