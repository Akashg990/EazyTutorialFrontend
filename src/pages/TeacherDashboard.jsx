...
import { API } from '../config'; // Optionally move REACT_APP_API_URL to a central config
...
const TeacherDashboard = () => {
  ...
  const handleFeature = async (courseId) => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to feature course');
  ...
  if (loading) {
    return <div className="container" style={{ padding: '40px 20px' }}>Loading your dashboard...</div>;
  }
        ...
        <div className="course-grid" style={{ marginTop: '20px' }}>
          {myCourses.map(course => (
            <div
              ...
              style={{
                cursor: 'default',
                border: course.isFeatured ? '2px solid var(--primary-color)' : '1px solid #ddd',
              }}
            >
              {course.isFeatured && (
                <div style={{ background: 'var(--primary-color)', color: 'white', textAlign: 'center', padding: '4px', fontWeight: 'bold' }}>
                  Featured
                </div>
              )}
              ...
              <div className="card-content">
                ...
                <div className="teacher-card-actions" style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <button
